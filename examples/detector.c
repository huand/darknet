  ...
  for (i = nthreads; i < m + nthreads; i += nthreads) {
    ...
    for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
      ...
      if (coco) {
        ...
        print_imagenet_detections(fp, i + t - nthreads + 1, dets, num, classes,
                                  w, h);
      } else {
...
void validate_detector(char *datacfg, char *cfgfile, char *weightfile,
  ...
  for (i = nthreads; i < m + nthreads; i += nthreads) {
    ...
    for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
      ...
      int nboxes = 0;
      detection *dets =
          get_network_boxes(net, w, h, thresh, .5, map, 0, &nboxes);
...
void validate_detector_recall(char *cfgfile, char *weightfile) {
  ...
  for (i = 0; i < m; ++i) {
    ...
    int nboxes = 0;
    detection *dets =
        get_network_boxes(net, sized.w, sized.h, thresh, .5, 0, 1, &nboxes);
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile,
  ...
  while (1) {
    ...
    // sized2.h)/2), net->w, net->h); resize_network(net, sized.w, sized.h);
    layer l = net->layers[net->n - 1];
...
/*
void censor_detector(char *datacfg, char *cfgfile, char *weightfile, int
cam_index, const char *filename, int class, float thresh, int skip)
{
#ifdef OPENCV
    char *base = basecfg(cfgfile);
    network *net = load_network(cfgfile, weightfile, 0);
...
void extract_detector(char *datacfg, char *cfgfile, char *weightfile, int
cam_index, const char *filename, int class, float thresh, int skip)
{
#ifdef OPENCV
    char *base = basecfg(cfgfile);
...
void run_detector(int argc, char **argv) {
  ...
  // else if(0==strcmp(argv[2], "extract")) extract_detector(datacfg, cfg,
  // weights, cam_index, filename, class, thresh, frame_skip); else
  // if(0==strcmp(argv[2], "censor")) censor_detector(datacfg, cfg, weights,
  // cam_index, filename, class, thresh, frame_skip);
}
